...
class ProductJourneySlider extends HTMLElement {
  ...
  setupViewportDetection() {
    ...
    const checkViewport = () => {
      ...
      // - Element is visible in viewport (top is visible or above)
      // - Bottom hasn't scrolled completely out of view
      // - Element fills most/all of the viewport
      const topInView = rect.top >= -100; // Allow small negative to account for scroll momentum
      const bottomInView = rect.bottom >= windowHeight * 0.8; // Bottom is mostly visible
      const fillsViewport = rect.height >= windowHeight * 0.8; // Element is tall enough
      
      this.isFullyInView = topInView && bottomInView && fillsViewport;
      ...
      console.log('Viewport check:', {
        ...
        bottomInView,
        fillsViewport,
        isFullyInView: this.isFullyInView
      });
    };
